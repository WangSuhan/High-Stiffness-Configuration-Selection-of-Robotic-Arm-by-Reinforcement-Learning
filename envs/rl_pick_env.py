...
class RLPickEnv(gym.Env):
    ...
    def reset(self):
        ...
        for _ in range(1000):
            ...
            zpos_target = random.uniform(self.z_low_obs, self.z_high_obs)  # TODO 原z=0.01
    ...
    def _get_obs(self):
        ...
        return np.hstack((np.array(robot_end_effector_position).astype(np.float32), achieved_goal, target_pos))
    ...
    def step(self, action):
        ...
        # 获取当前机械臂末端坐标
        self.current_pos = np.array(p.getLinkState(self.kuka_id, self.end_effector_index)[4]).astype(np.float32)
        # 计算下一步的机械臂末端坐标
        self.new_robot_pos = [
    ...
    def _reward(self):
        ...
        robot_end_effector_position = np.array(p.getLinkState(self.kuka_id, self.end_effector_index)[4]).astype(np.float32)
        ...
        self.distance_robot = np.linalg.norm(robot_end_effector_position - self.object_state, axis=-1)
        # print(self.distance)
        ...
        self.observation = robot_end_effector_position
