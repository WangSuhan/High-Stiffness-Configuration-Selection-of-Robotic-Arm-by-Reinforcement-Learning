...
def run(**kwargs):
    ...
    # 建立env python main.py run --env=RLReachEnv --algo=DADDPG_MLP --vis_name=Reach_DADDPG
    env = getattr(envs, opt.env)(is_render=True, is_good_view=False)
    # 获取环境中的动作空间、状态空间、动作限幅
    state_dim = env.observation_space.shape[0]+3
    action_dim = env.action_space.shape[0]
    action_bound = float(env.action_space.high[0]) + 0.1
    # 随机种子使结果可复现
        ...
        with tqdm(total=int(opt.num_episodes / 10), desc='Iteration %d' % i) as pbar:
            ...
            for i_episode in range(int(opt.num_episodes / 10)):
                ...
                if (i_episode + 1) % 25 == 0:
                    ...
                    if rate >= max_rate:
                        agent.save(f"D:/book/first semester/IRMAC/20240318/DRL-on-robot-arm-master/result/{rate}")
                        max_rate = rate
...
def train_reach_with_DDPG(**kwargs):
    ...
    # action_bound = float(env.action_space.high[0]) + 0.3
    action_dim = 6
    action_bound = float(env.action_space.high[0]) + 1
        ...
        with tqdm(total=int(opt.num_episodes / 10), desc='Iteration %d' % i) as pbar:
            ...
            for i_episode in range(int(opt.num_episodes / 10)):
                ...
                if (i_episode + 1) % 25 == 0:
                    ...
                    vis.plot("success_rate", rate)
                    if rate >= max_rate:
                        agent.save(f"D:/book/first semester/IRMAC/20240318/DRL-on-robot-arm-master/result/push/{rate}")
                        max_rate = rate
                        opt.her_ratio = opt.her_ratio * 0.75
                        # opt.expl_noise = opt.expl_noise * 0.75
                    rate = 0.0
